...
const ShowDetailsRowDaily = ( { day } ) => {
  ...
  const high = showTemperature( high_temp );
  const low = showTemperature( low_temp );
  ...
  return (
    <tr >
      ...
      <td>
        <span className="high-temperature">{ high }</span>°/
        <span className="forecast-smaller"> { low }°</span>
      </td>
      <td>&nbsp;
        <Icon icon= { icon }/>
      </td>
